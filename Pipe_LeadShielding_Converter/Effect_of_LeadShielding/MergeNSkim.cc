...
int main(){
	...
	for (int i=1; i<=19590; i++){
		...
		filename << "Pipe_Lead_WO_FIBRE_" << i << ".root";
		rootchain->Add(filename.str().c_str());
		if (i%1000==0) cout << "Merging files..." << endl;
	}
	rootchain->Merge("allfiles_Pipe_Lead_WO_FIBRE.root");
...
//	rootchain->SetBranchAddress("Reflectiony",&y);
	...
	TFile * skimfile = new TFile("allfiles_Pipe_Lead_WO_FIBRE_skimmed.root","RECREATE");
	...
	//	if (particle_id==PDGID_GAMMA && hasLeft_status==true) continue;
		...
		newtree->Fill();
		if (k%1000==0) cout << "Skimming file..." << endl;
	}

...
int main(){
	...
	for (int i=1; i<=19590; i++){
		...
		filename << "Pipe_AluWindow_Converter_MAGNET_wo_Collimator_" << i << ".root";
		rootchain->Add(filename.str().c_str());
		if (i%1000==0) cout << "Merging files..." << endl;
	}
	rootchain->Merge("allfiles_Magnet_wo_Collimator.root");
...
//	rootchain->SetBranchAddress("Reflectiony",&y);
	...
	TFile * skimfile = new TFile("allfiles_Magnet_wo_Collimator_skimmed.root","RECREATE");
	...
	//	if (particle_id==PDGID_GAMMA && hasLeft_status==true) continue;
		...
		newtree->Fill();
		if (k%5000==0) cout << "Skimming file..." << endl;
	}
